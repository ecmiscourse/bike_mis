...
<script type="text/javascript">
	$(function(){
		...
		$("input",$("#personSn").next("span")).blur(function(){
			...
			if($('#personSn').textbox('isValid')&&$('#personSn').textbox('getValue')!=row.personSn){
				$.ajax({
		...
		$('#submit').click(function(){
			...
			if($('#ff').form('validate')){
				$('#ff').form('submit', {    
				    url:'${pageContext.request.contextPath}/base/personAction_update.action', 
				    queryParams:{'oldPersonSn':row.personSn},      
				    success:function(data){    
