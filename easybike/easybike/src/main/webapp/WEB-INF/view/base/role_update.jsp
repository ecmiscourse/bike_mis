...
<script type="text/javascript">
	$(function(){
		...
		$("input",$("#roleSn").next("span")).blur(function(){
			...
			if($('#roleSn').textbox('isValid')&&$('#roleSn').textbox('getValue')!=row.roleSn){
				$.ajax({
					...
					success:function(data){
						if(data.isExist==true){
							parent.$.messager.alert('我的提示','该角色编号已经存在！','warning');
		...
		$('#submit').click(function(){
			...
			if($('#ff').form('validate')){
				$('#ff').form('submit', {    
				    url:'${pageContext.request.contextPath}/base/roleAction_update.action',       
				    queryParams:{'oldRoleSn':row.roleSn},  
				    success:function(data){
				    	$("#ff").form("disableValidation");    
